...
module.exports = {
  ...
  create(context) {
    ...
    function formatComment(indent) {
      return c => (
        c.type === 'Line' ?
          makeSpaceOf(indent) + '//' + c.value + '\n'
        :
          '/*' + c.value + '*/'
      )
    }
    ...
    function rewriteTernary(fixer, node, baseIndent, preText, postText) {
      return (
        replaceBetweenIncl(
          ...
          + " ?\n"
          + sourceCode.getComments(node.consequent).leading.map(formatComment(baseIndent + indent)).join('')
          + makeSpaceOf(baseIndent + indent)
          + sourceCode.getText(node.consequent)
          + sourceCode.getComments(node.consequent).trailing.map(formatComment(baseIndent + indent)).join('')
          + "\n"
          + makeSpaceOf(baseIndent)
          + ":\n"
          + sourceCode.getComments(node.alternate).leading.map(formatComment(baseIndent + indent)).join('')
          + makeSpaceOf(baseIndent + indent)
          + sourceCode.getText(node.alternate)
          + sourceCode.getComments(node.alternate).trailing.map(formatComment(baseIndent + indent)).join('')
          + (postText || "")
        )
      )
    }
